...
<template>
  <div>
    <div
      v-if="sneakerStore.show"
      class=""
      :class="{
        'cartVisible transition-opacity duration-700 opacity-100':
          sneakerStore.show,
        'cart opacity-0 transition-opacity  duration-700': !sneakerStore.show,
      }"
    >
      <div
          ...
          <button
            ...
            @click="
              (useCookie('pageT').value = 'classic'),
                ...
                consoleMethod()
            "
            :disabled="cartStore.localCounter <= 0"
          >
            Оформить Заказ
          </button>
        </div>
