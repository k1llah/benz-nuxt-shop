	...
	<div>
		...
		<div class="md:mt-[100px] sm:mt-10 flex justify-around sm:flex-wrap p-5">
			...
			<div class="flex flex-col gap-7 max-w-[370px] sm:mt-10 md:mt-0 p-4">
				<p
					class="md:text-3xl sm:text-xl text-[#000000] dark:text-ghostWhiteText font-sans font-[600]"
				>
					{{ itemData?.title }}
				</p>
				...
				<div class="flex md:gap-5 sm:gap-2 mt-9 md:flex-nowrap sm:mt-2">
					<button
						...
						@click="
							;(useCookie('pageT').value = 'fast'), (orderStore.targetPage = 'fast'), getPathName()
						"
					>
						Быстрый заказ
						<span
		...
		<div class="flex p-7 pb-16 justify-end mt-10">
			<h4 class="text-base text-slate-400 text-500 text-right max-w-[500px] dark:text-orange-500">
				Качество товара, его безопасность для жизни, здоровья потребителей, окружающей среды,
				подтверждено в соответствии с требованиями законодательства. Подробная информация размещена
				на этикетке или упаковке товара, либо в технической документации к нему.
			</h4>
		</div>
	</div>
</template>
