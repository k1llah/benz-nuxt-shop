	...
	const submitForm = async (event: Event) => {
		...
		try {
			if (isFormChanged.value == true && correctDataInput.value == true) {
				...
				nameWarning.value = 'Имя содержит пробел, спецсимвол или оно меньше 2 символов'
			} else if (
				email.value.includes(' ') ||
				email.value.length < 2 ||
				!email.value.includes('@')
				...
				emailWarning.value = 'email содержит пробел, меньше 2 символов или не содержит @'
			} else {
...
<template>
	<div class="flex gap-5 items-center">
		...
		<h2 class="md:text-3xl md:text-[unset] sm:text-xl sm:text-right dark:text-ghostWhiteText">
	...
	<div class="m-auto flex flex-col items-center mt-10 gap-9">
		...
		>
			<div>
				...
				<label class="pt-1 block text-red-700 text-sm max-w-52 transition-all duration-500">{{
					lastNameWarning
				}}</label>
			</div>
			<div>
				...
				<label class="pt-1 block text-red-700 text-sm max-w-52">{{ nameWarning }}</label>
			</div>
			<div>
				<label
					for="inputEmail"
				...
				<label class="pt-1 block text-red-700 text-sm max-w-52">{{ emailWarning }}</label>
			</div>
